...
class TransformerEncoderLayer(nn.Module):
    ...
    # input 加上 positional encoding
    def with_pos_embed(self, tensor, pos: Optional[Tensor]):
        return tensor if pos is None else tensor + pos

    # Transformer Encoder block 里的核心模块（multi-head-attention -> layerNorm -> residual-> MLP -> layerNorm -> residual）
    # 和 detr 的 forward 方法一致
...
class TransPoseR(nn.Module):
    ...
    def forward(self, x):
        ...
        bs, c, h, w = x.shape # 10, 256, 32, 24

        # 将经过 backbone 提取后的 feature map 信息 flatten 之后输入 encoder
        x = x.flatten(2).permute(2, 0, 1) # 768, 10, 256
        x = self.global_encoder(x, pos=self.pos_embedding) # 768, 10, 256
        x = x.permute(1, 2, 0).contiguous().view(bs, c, h, w) # 10, 256, 32, 24
        x = self.deconv_layers(x) # 10, 256, 64, 48 (反卷积)
        x = self.final_layer(x) # 10, 21, 64, 48
